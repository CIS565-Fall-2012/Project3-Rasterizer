...
float Camera::dfltSpeed = (float)0.1;
...
void Camera::reset()
{
   ...
   mPitch = -asin(dfltEye[1]/length(dfltEye));
   mHeading = acos(dfltEye[0]/length(dfltEye)*cos(mPitch));
   //printf("INIT: %f %f\n", mPitch, mHeading);
...
void Camera::set(const vec3& eyepos, const vec3& look, const vec3& up)
{
   ...
   v = cross(up,n);
   u = cross(n,v);
   mRadius = length(n); // cache this distance
